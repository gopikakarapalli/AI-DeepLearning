...
num_classes = 4
dataset=pickle.load(open('../Temp/imagedata_set.pickle','rb'))
print(dataset)
x,y =dataset
...
filename='../Temp/model_train_callbacks.csv'
csv_log=callbacks.CSVLogger(filename, separator=',', append=False)
early_stopping=callbacks.EarlyStopping(monitor='val_loss', min_delta=0, patience=0, verbose=0, mode='min')

filepath="../Temp/model-weights-{epoch:03d}-{loss:.4f}-{acc:.4f}.hdf5"
...
plt.savefig("../Temp/featuremaps-layer-{}".format(layer_num) + "-filternum-{}".format(filter_num)+'.jpg')

num_of_featuremaps=feature_maps.shape[2]
fig=plt.figure(figsize=(16,16))	
plt.title("../Temo/featuremaps-layer-{}".format(layer_num))
...
for i in range(int(num_of_featuremaps)):
	...
	plt.tight_layout()
plt.show()
fig.savefig("../Temp/featuremaps-layer-{}".format(layer_num) + '.jpg')
...
with open("../Temp/model.json", "w") as json_file:
